...
class Domain {
   ...
   public:
            ...
            for(int leader_point = 0; leader_point < num_leaders; leader_point++)
            {
                ...
                for (auto& point : points) {
                    ...
                    double noise_variance =
                        params.noise ?
                            1 + fmod(rand_float, params.noise) :
                            1;
    ...
    void generate_paths(const DomainParameters& params, unsigned number_of_paths) {
...
std::pair<std::shared_ptr<Domain>, std::shared_ptr<DomainParameters>> setup_domain(const std::string& filename) {
